...
from model import Crop, DeNormalize, Face_Inversion, CELEBA_DP_Classifier, Face_Inversion_noise
...
def main():
    ...
    if config.arch == 'face_cnn':
        model = CELEBA_DP_Classifier(resnet18(pretrained=False), config.split_index, config.noise_scale)
    else:
