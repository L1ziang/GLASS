...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # classifier = getattr(self, 'classifier' + str(0).zfill(2))
        # y = classifier(x)        
    ...
    def getLayerOutputFrom(self, x, targetLayer):
        back_flag = False
        for name, module in self._modules.items():
            if back_flag == False and targetLayer[0] == name:
                back_flag = True
                continue
            if back_flag:
                if name == 'stem':
...
class Encoder_network(nn.Module):
    ...
    def __init__(self, block, layers, attack_layer, num_attributes=1, zero_init_residual=False):
        ...
        self.Conv2 = nn.Conv2d(512, 512, kernel_size=2)
    ...
    def forward(self, x, realimg=False):
        if realimg == True:
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
...
class Topaz(nn.Module):

    def __init__(self, block, layers, num_attributes=1, zero_init_residual=False):
        super(Topaz, self).__init__()
        self.inplanes = 3
        self.block1 = self._make_layer(block, 3, layers[0])
...
def topaz(**kwargs):
    model = Topaz(BasicBlock, [1], **kwargs)
    return model
