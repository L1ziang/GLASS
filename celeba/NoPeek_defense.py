...
from model import Crop, DeNormalize, Face_Inversion, CELEBA_Dropout_Classifier, Face_Inversion_noise, CELEBA_NoPeek_Classifier
...
def main():
    ...
    if config.arch == 'face_cnn':
        model = CELEBA_NoPeek_Classifier(resnet18(pretrained=False), config.split_index)
    else:
    ...
    if config.inv:
        ...
        # attack_train_data = Attack_Dataset()
    ...
    if config.dp:
        ...
        criterion = NoPeek_Loss(dcor_weighting=config.dcor_weighting)
    ...
    for epoch in range(start_epoch, config.epochs+1):
        ...
        if config.dp:
            ...
            if test_acc > best_test_acc:
                ...
                print ("new best acc is {}%, saving best model\n".format(best_test_acc))
